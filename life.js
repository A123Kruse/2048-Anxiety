// Conway-like background FX used by the game via global `life`
(() => {
  ...
  const life = (() => {
    ...
    const PALETTE = [
      [255,199,120],[255,170,110],[237,145,90],[205,130,85],[255,214,140]
    ];
    ...
    function mapCell(r, c){
      const x = Math.round((c + 0.5) * (cols / SIZE));
      const y = Math.round((r + 0.5) * (rows / SIZE));
      return {x,y};
    }

    function setAlive(x, y){
      const ix = ((y + rows) % rows) * cols + ((x + cols) % cols);
      a[ix] = 1;
    }
    function setBlinker(cx, cy, hor){
      if (hor){ setAlive(cx-1,cy); setAlive(cx,cy); setAlive(cx+1,cy); }
      else { setAlive(cx,cy-1); setAlive(cx,cy); setAlive(cx,cy+1); }
    }
    function seedDisk(cx, cy, r=3, d=0.6){
      for (let y=-r; y<=r; y++){
        for (let x=-r; x<=r; x++){
          if (x*x + y*y <= r*r && Math.random() < d){
            setAlive(cx+x, cy+y);
          }
        }
      }
      for (let i=0;i<2;i++){
        const ox = cx + (Math.random()*6 - 3 | 0);
        const oy = cy + (Math.random()*6 - 3 | 0);
        setBlinker(ox, oy, Math.random() < 0.5);
      }
    }
    ...
    function draw(){
      ...
      ctx.fillStyle = `rgba(187,173,160,${FADE_ALPHA})`; // toward board color
      ctx.fillRect(0,0,w,h);
      ctx.fillStyle = `rgba(${r},${g},${b_},0.8)`;
      for (let y=0;y<rows;y++){
        for (let x=0;x<cols;x++){
          if (a[y*cols+x]) ctx.fillRect(x*cellSize,y*cellSize,cellSize,cellSize);
        }
      }
    }
    ...
    function burstAtCell(r,c){
      const {x,y} = mapCell(r,c);
      seedDisk(x,y, 3 + (Math.random()*2|0), 0.55);
    }
    function init(){
      resize();
    ...
    window.addEventListener('resize', () => {
      const was = rafId != null;
      if (was) cancelAnimationFrame(rafId);
      resize();
      if (was) rafId = requestAnimationFrame(loop);
    });
